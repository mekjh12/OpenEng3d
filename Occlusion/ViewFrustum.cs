...
namespace Occlusion
{
    ...
    public class ViewFrustum
    {
        ...
        // 최적화용 변수
        public static Polyhedron _polyhedron = new Polyhedron();
        public static Edge[] _edges = new Edge[12];
        public static Face[] _faces = new Face[6];
        public static Plane[] _planes = new Plane[6];
        ...
        public static Polyhedron BuildFrustumPolyhedron(Camera camera, float scaled = 0.6f)
        {
            float g = 1.0f / (float)MathFast.Tan((camera.FOV * scaled).ToRadian());
            return ViewFrustum.BuildFrustumPolyhedronOptima(
                    camera.Position,
                    camera.Forward,
                    camera.Up,
                    camera.Right,
        ...
        float g, float s, float n, float f)
        {
            ...
            {
                // 수직 모서리 설정
                _edges[i + 0].SetVertexIndex(0, (byte)i);
                _edges[i + 0].SetVertexIndex(1, (byte)(i + 4));
                _edges[i + 0].SetFaceIndex(0, (byte)i);
                _edges[i + 0].SetFaceIndex(1, (byte)((i - 1) & 3));

                // 근평면 모서리 설정
                _edges[i + 4].SetVertexIndex(0, (byte)i);
                _edges[i + 4].SetVertexIndex(1, (byte)((i + 1) & 3));
                _edges[i + 4].SetFaceIndex(0, 4);
                _edges[i + 4].SetFaceIndex(1, (byte)i);

                // 원평면 모서리 설정
                _edges[i + 8].SetVertexIndex(0, (byte)(((i + 1) & 3) + 4));
                _edges[i + 8].SetVertexIndex(1, (byte)((i + 4)));
                _edges[i + 8].SetFaceIndex(0, 5);
                _edges[i + 8].SetFaceIndex(1, (byte)(i));

                // 측면 면 설정
                _faces[i].EdgeCount = 4;
                _faces[i].SetEdge(0, (ushort)(i));
                _faces[i].SetEdge(1, (ushort)((i + 1) & 3));
                _faces[i].SetEdge(2, (ushort)(i + 4));
                _faces[i].SetEdge(3, (ushort)(i + 8));
            }

            // 근평면과 원평면 설정
            _faces[4].EdgeCount = 4; _faces[5].EdgeCount = 4;
            _faces[4].SetEdge(0, 4); _faces[5].SetEdge(0, 8);
            _faces[4].SetEdge(1, 5); _faces[5].SetEdge(1, 9);
            _faces[4].SetEdge(2, 6); _faces[5].SetEdge(2, 10);
            _faces[4].SetEdge(3, 7); _faces[5].SetEdge(3, 11);

            polyhedron.SetEdge(_edges);
            polyhedron.SetFace(_faces);
        ...
        /// <param name="f">원평면 거리</param>
        /// <returns>생성된 뷰 프러스텀 다면체</returns>
        public static Polyhedron BuildFrustumPolyhedronOptima(
        Vertex3f cameraPos, Vertex3f cameraForward, Vertex3f cameraUp, Vertex3f cameraRight,
        float g, float s, float n, float f)
        {
            // 재사용을 위해 정적 멤버 사용
            if (_polyhedron == null) _polyhedron = new Polyhedron();
            ...
            vertices[6] = (cameraView * new Vertex4f(-x, -y, f, 1)).xyz();  // 좌하단
            vertices[7] = (cameraView * new Vertex4f(-x, y, f, 1)).xyz();   // 좌상단
            _polyhedron.SetVertex(vertices);

            // 6개의 평면 생성 (4개의 측면 + 근평면 + 원평면)
            Matrix4x4f inverse = cameraView.Transposed;

            // 4개의 측면 평면 생성
            _planes[0] = new Plane((vertices[4] - vertices[0]).Cross(vertices[1] - vertices[0]).Normalized, vertices[0]); // 우측면
            _planes[1] = new Plane((vertices[5] - vertices[1]).Cross(vertices[2] - vertices[1]).Normalized, vertices[1]); // 하단면
            _planes[2] = new Plane((vertices[6] - vertices[2]).Cross(vertices[3] - vertices[2]).Normalized, vertices[2]); // 좌측면
            _planes[3] = new Plane((vertices[7] - vertices[3]).Cross(vertices[4] - vertices[3]).Normalized, vertices[3]); // 상단면

            // 근평면과 원평면 생성
            Vertex3f col2 = cameraView.Column2.xyz();
            Vertex3f col3 = cameraView.Column3.xyz();
            float d = col2.Dot(col3);
            _planes[4] = new Plane(col2, -(d + n));         // 근평면
            _planes[5] = new Plane(-col2, col3 + col2 * f); // 원평면

            _polyhedron.SetPlane(_planes);
            ...
            _polyhedron.SetEdge(_edges);
            _polyhedron.SetFace(_faces);
