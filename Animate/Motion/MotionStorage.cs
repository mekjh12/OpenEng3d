...
namespace Animate
{
    public class MotionStorage
    {
        ...
        public Motionable DefaultMotion
        {
            get
            {
                // 가장 빠른 방법: foreach로 첫 번째 요소만 가져오기
                foreach (Motionable motion in _motions.Values)
                {
        ...
        public Motionable GetMotion(HUMAN_ACTION motion)
        {
            return (_motions.ContainsKey(motion.ToString())) ? _motions[motion.ToString()] : null;
        }

        public Motionable GetMotion(string motionName)
        {
