...
vec4 TerrainBlending(float height, sampler2D tex1, sampler2D tex2, sampler2D tex3, vec2 uv)
{
    // 높이에 따른 블렌딩 가중치 계산
    float w1 = smoothstep(0.0, 0.3, height);
    float w2 = smoothstep(0.25, 0.6, height) * (1 - smoothstep(0.5, 0.8, height));
    float w3 = smoothstep(0.55, 0.9, height);
    ...
    // 텍스처 블렌딩
    return texture(tex1, uv) * w1 + texture(tex2, uv) * w2 + texture(tex3, uv) * w3;
}
...
{   
   ...
   // 높이맵에서 높이값 샘플링
   Height = SampleHeightForChunk(Tex3, blendFactor, heightMapHighRes, heightMapLowRes,
   ...
   float detail = texture(gDetailMap, Tex3).r;
   p.z = 0.9f * p.z + detail * 0.1f;
