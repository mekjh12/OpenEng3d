...
void main()
{
    ...
    // 테셀레이션 레벨 범위
    const int MIN_TESS_LEVEL = 16;    // 최소 테셀레이션 분할 수
    const int MAX_TESS_LEVEL = 64;   // 최대 테셀레이션 분할 수

    // 단계 4: 각 엣지의 테셀레이션 레벨 계산
    // 각 엣지에서 가까운 정점의 거리를 기준으로 보간
    float TessLevel0 = mix(MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance10, Distance00));  // 좌측 엣지
    float TessLevel1 = mix(MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance00, Distance01));  // 하단 엣지
    float TessLevel2 = mix(MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance01, Distance11));  // 우측 엣지
    float TessLevel3 = mix(MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance11, Distance10));  // 상단 엣지

    // 단계 5: 외곽 엣지 테셀레이션 레벨 설정
