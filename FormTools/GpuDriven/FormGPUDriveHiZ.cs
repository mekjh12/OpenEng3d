    ...
    public partial class FormGPUDriveHiZ : Form, GlControlerable
    {
        ...
        public void UpdateFrame(int deltaTime, int width, int height, Camera camera)
        {
            ...
            if (_prevCameraPosition != camera.Position)
            {
                ...
                _hzbuffer.RenderTerrainDepth(TerrainConstants.DEFAULT_VERTICAL_SCALE,
            ...
            _gpuDriven.GetVisibleCountDebug(ref _visibleCount, ref _frustumPassCount);

            if (_visibleCount != _lastVisibleCount || _frustumPassCount != _lastFrustumPassCount)
            {
                // 네임플레이트 업데이트            
                _textNamePlate.Text = $"{_visibleCount}/{_frustumPassCount}";
